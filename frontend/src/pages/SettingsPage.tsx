...
import { Save, Building2, CreditCard, Moon, Sun, Mail, Users, Settings as SettingsIcon } from 'lucide-react'
...
export default function SettingsPage() {
    ...
    const { data: settingsData, isLoading } = useQuery({
        queryKey: ['settings'],
        queryFn: async () => {
            // Need token for settings now too? Yes because we protected it!
    ...
    // Auth headers are needed for fetching settings now
    if (isLoading && activeTab === 'general') return <div className="p-8">Cargando...</div>
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <div className="mb-6 flex justify-between items-end">
                ...
                <div className="flex bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-1 rounded-lg shadow-sm">
                    <button
                        onClick={() => setActiveTab('general')}
                        className={clsx(
                            "px-4 py-2 text-sm font-medium rounded-md transition-colors flex items-center gap-2",
                            activeTab === 'general' ? "bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white" : "text-slate-500 hover:text-slate-900 dark:hover:text-slate-300"
                        )}
                    >
                    ...
                    <button
                        ...
                        className={clsx(
                            "px-4 py-2 text-sm font-medium rounded-md transition-colors flex items-center gap-2",
                            activeTab === 'users' ? "bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white" : "text-slate-500 hover:text-slate-900 dark:hover:text-slate-300"
                        )}
                    >
                        <Users size={16} /> Usuarios y Seguridad
                    </button>
                </div>
            </div>
